...
class SpeechAPISample:
    ...
    def sendAudioFile(self, apiName, seqValue, finished, filePath, compressed): 
        ...
        return self.sendAudioData(apiName, seqValue, finished, bAudioData, compressed)
    
    def sendAudioData(self, apiName, seqValue, finished, bAudioData, compressed):
    ...
    def sendApiRequest(self, apiName, rq):
        ''' composite post data field''' 
        postData = str(self.getBasicQueryString(apiName, ''))
        print('rq=%s' % json.dumps(rq))
        postData += '&rq=' + quote_plus(json.dumps(rq))
        
        ''' Request speech recognition service by HTTP POST '''
        url = str(self.apiBaseUrl)
        headers = { 'Connection'    : "Keep-Alive",
